...
# Touch tracking for mobile multi-touch (pinch zoom & pan)
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event is InputEventScreenTouch:
		...
		if touches.size() == 1:
			# Single finger drag (only move camera if not currently placing PVO tower)
			...
			if initial_touch_dist > 5.0 and current_dist > 5.0:
				...
				# Focus zoom around the midpoint between fingers
				var midpoint: Vector2 = (p0 + p1) * 0.5
				_zoom_at_viewport_point(new_zoom_val, midpoint)

			# Pan using finger movement
			position -= (event.relative * 0.5) / zoom

	# --- Mouse handling for desktop/emulator ---
...
func _zoom_at_viewport_point(new_zoom_val: float, viewport_point: Vector2) -> void:
	target_zoom = Vector2(new_zoom_val, new_zoom_val)
