...
func _process(delta: float) -> void:
	...
	if prep_progress < 35.0:
		status_label.text = "Подготовка к учёбе..." if is_ru else ("Підготовка сцени..." if is_uk else "Preparing for studies...")
	elif prep_progress < 70.0:
		status_label.text = "Загрузка учёбы..." if is_ru else ("Завантаження навчання..." if is_uk else "Loading study materials")
	elif prep_progress < 100.0:
		status_label.text = "Развёртывание учебника..." if is_ru else ("Розгортання підручника" if is_uk else "Expanding the textbook")
	else:
		status_label.text = "Помощь готова!" if is_ru else ("Допомога готова!" if is_uk else "Help is ready!")
